...
os.system('mkdir -p %s_results/'%(prefix))
...
for sgRNA in removed_sgRNALst:
    g.write(sgRNA+'\n')
g.close()
os.system('mkdir -p %s_results/removed.sgRNA/'%(prefix))
...
os.system('mkdir -p %s_results/replicate_consistence/'%(prefix))
...
for i,condition in enumerate(stressed_conditionLst):
    ...
    g.write('%s\t%s\t%s\t%s\n'%(condition_name,str(medianZ),str(meanZ),str(stdZ)))
g.close()
os.system('mkdir -p %s_results/NCsgRNA_ND/'%(prefix))
...
os.system('mkdir -p %s_results/%s_sgRNA_statistics/'%(prefix,prefix))
os.system('mkdir -p %s_results/%s_sgRNA_statistics/library_level/'%(prefix,prefix))
for library in LibraryLst:
...
os.system('mkdir -p %s_results/%s_sgRNA_statistics/condition_level/'%(prefix,prefix))
for condition in ConditionLst:
...
os.system('mkdir -p %s_results/%s_sgRNA_statistics/combined_condition_level/'%(prefix,prefix))
for condition in stressed_conditionLst:
    condition_name=stressed_conditionDic[condition]
