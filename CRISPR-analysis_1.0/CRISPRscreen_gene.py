...
def quasiGene_construct(NCsgRNADic,sgRNAperQuasiDown,sgRNAperQuasiUp,Quasi_number,Testapproach):
    ...
    for sgRNAnumber in range(sgRNAperQuasiDown,(sgRNAperQuasiUp+1)):
        ...
        for gene in range(Quasi_number):
            ...
            FDRdic[sgRNAnumber].append(score)
...
# Process the NCsgRNA_scoreDic to calculate the FDR fitting and do plotting (CDF)
# store score-FDR function
NCsgRNA_FDRDic={}
# store FDR-score function
NCsgRNA_FDRrevDic={}
...
for condition in stressed_conditionLst:
    ...
    for sgRNAnumber in NCsgRNA_scoreDic[condition]:
        ...
        if N!=(Quasi_number+2):
            ...
            NCsgRNA_FDRrevDic[condition][sgRNAnumber]['Interpolation']=Rev_interpolationFunction
        if sgRNAnumber in [1,2,3,5,10,15,20,25,30,40]:
            sgRNAnumber_strLst.append('sgRNAnumber= '+str(sgRNAnumber))
            color=colors[[1,2,3,5,10,15,20,25,30,40].index(sgRNAnumber)]
...
for sgRNAnumber in range(sgRNAperQuasiDown,(sgRNAperQuasiUp+1)):
    ...
    for condition in stressed_conditionLst:
        ...
        for FDR in [0.001,0.005,0.01,0.02,0.05,0.1]:
            score=NCsgRNA_FDRrevDic[condition][sgRNAnumber]['Interpolation'](FDR)
            myFDRbook[condition][FDR]=score
    myFDR_DF=pd.DataFrame(myFDRbook)
    myFDR_DF['FDR']=myFDR_DF.index
    myFDR_DF.to_csv('%s_results/%s_quasigeneFDR/FDR_sgRNAnumber=%d.txt'%(prefix,prefix,sgRNAnumber),index=None,sep='\t')
        ...
        if gene!='0':
            ...
            if hit_gene_calling=='all':
                ...
                if sgRNAnumber>=1:
                    ...
                    sgRNA_Lst=[x for x in Processed_geneDic[gene][condition]]
                    ...
                    score=abs(medianRAC)*(-1*Log10P)
                    FDRvalue=NCsgRNA_FDRDic[condition][sgRNAnumber]['Interpolation'](score)
                else:
                    print ('not enough sgRNAs for gene %s in control condition %s'%(gene,condition))
                    medianRAC=0; medianZ=0; Log10P=0; sgRNAnumber=0; FDRvalue=1
            ...
            # if position-priority based subsampling is used, then search the peak of significance and store the values into the dic if better results detected
                ...
                Result_Dic[condition][gene]['Log10Pvalue']=0
                Result_Dic[condition][gene]['sgRNAnumber']=0
                Result_Dic[condition][gene]['FDRvalue']=1
                if available_sgRNAnumber>=1:
                    # get all available sgRNAs for this gene passing the control condition read number threshold
                    mysgRNA_Lst=[x for x in Processed_geneDic[gene][condition]]
                    # get the sgRNAs from the position ordered list and filter those <=readthreshold in control condition
                    thisgene_sgRNApositionOrderLst=[sgRNA for sgRNA in gene_sgRNApositionOrderDic[gene] if sgRNA in mysgRNA_Lst]
                    ...
                    for sgRNAnumber in range(temporary_sgRNAnumber,available_sgRNAnumber+1):
                        ...
                        score=abs(medianRAC)*(-1*Log10P)
                        FDRvalue=NCsgRNA_FDRDic[condition][sgRNAnumber]['Interpolation'](score)
                        if FDRvalue<Result_Dic[condition][gene]['FDRvalue']:
                            Result_Dic[condition][gene]['MedianRAC']=medianRAC
                            Result_Dic[condition][gene]['MedianZ']=medianZ
                            Result_Dic[condition][gene]['Log10Pvalue']=Log10P
                            Result_Dic[condition][gene]['sgRNAnumber']=sgRNAnumber
