...
def ZscoreExtract(sgRNALst,sgRNA_Zscore_dic):
    ZscoreLst=[]
    for sgRNA in sgRNALst:
        Zscore=sgRNA_Zscore_dic[sgRNA]
        ZscoreLst.append(Zscore)
    return ZscoreLst
...
def quasiGene_construct(NCsgRNADic,sgRNAperQuasiDown,sgRNAperQuasiUp,Quasi_number,Testapproach):
    ...
    for sgRNAnumber in range(sgRNAperQuasiDown,(sgRNAperQuasiUp+1)):
        ...
        for gene in range(Quasi_number):
            ...
            ZscoreLst_shuffle=ZscoreExtract(sgRNALst_shuffle,NCsgRNADic)
            # calculate the parameter
            medianZ_shuffle=np.median(np.array(ZscoreLst_shuffle))
            Ustatistic,pValue_shuffle=mannwhitneyu(ZscoreLst_shuffle,NC_relative_abundance_changeLst,True,Testapproach)
...
# {condition1:[Zscore1,Zscore2, ..], condition2:[], ..}
...
for condition in stressed_conditionLst:
    ...
    plt.xlabel('Log10 MWU test P value')
    plt.ylabel('False discovery rate')
    plt.xlim((-3,-0.5))
    plt.savefig('%s_results/%s_%s_quasigeneFDR.png'%(prefix,prefix,condition))
    plt.clf()
...
Result_geneDic={}
# {gene:{'Median':xx,'Log10Pvalue':xx,'sgRNAnumber':xx,'FDRvalue':xx}, gene:{}, ..}


for condition in stressed_conditionLst:
    Result_geneDic[condition]={}
    for gene in processed_geneLst:
        if gene!='0':
            Result_geneDic[condition][gene]={}
            ...
            if hit_gene_calling=='all':
                ...
                relativechangeLst=ZscoreExtract(sgRNALst,Processed_geneDic[gene][condition])
                medianZ=np.median(np.array(relativechangeLst))
                ...
                Result_geneDic[condition][gene]['Median']=medianZ
                Result_geneDic[condition][gene]['Log10Pvalue']=Log10P
                Result_geneDic[condition][gene]['sgRNAnumber']=sgRNAnumber
                Result_geneDic[condition][gene]['FDRvalue']=FDRvalue
            # if position-priority based subsampling is used, then search the peak of significance and store the values into the dic if better results detected
                ...
                Result_geneDic[condition][gene]['Median']=0
                Result_geneDic[condition][gene]['Log10Pvalue']=1
                Result_geneDic[condition][gene]['sgRNAnumber']=sgRNAperQuasiDown
                Result_geneDic[condition][gene]['FDRvalue']=1
                if min_sgRNAnumber<available_sgRNAnumber+1:
                    temporary_sgRNAnumber=min_sgRNAnumber
                else:
                    print('The gene do not has %s sgRNAs'%(min_sgRNAnumber))
                ...
                for sgRNAnumber in range(temporary_sgRNAnumber,available_sgRNAnumber+1):
                    ...
                    relativechangeLst=ZscoreExtract(sgRNALst_position,Processed_geneDic[gene][condition])
                    medianZ=np.median(np.array(relativechangeLst))
                    Ustatistic,pValue=mannwhitneyu(relativechangeLst,NC_relativechangeLst,True,Testapproach)
                    Log10P=math.log(pValue,10)
                    FDRvalue=NCsgRNA_FDRDic[condition][sgRNAnumber]['Interpolation'](Log10P)
                    if Log10P<Result_geneDic[condition][gene]['Log10Pvalue']:
                        Result_geneDic[condition][gene]['Median']=medianZ
                        Result_geneDic[condition][gene]['Log10Pvalue']=Log10P
                        Result_geneDic[condition][gene]['sgRNAnumber']=sgRNAnumber
                        Result_geneDic[condition][gene]['FDRvalue']=FDRvalue

pickle.dump(Result_geneDic,open('%s_results/%s_Result_geneDic.pickle'%(prefix,prefix),'wb'))  #
...
for condition in stressed_conditionLst:
    ...
    for gene in processed_geneLst:
        if gene!='0':
            ...
            g.write('%s\t%s\t%s\t%s\t%s\n'%(gene,str(medianZ),str(-1*Log10P),str(sgRNAnumber),str(FDRvalue)))
    g.close()
