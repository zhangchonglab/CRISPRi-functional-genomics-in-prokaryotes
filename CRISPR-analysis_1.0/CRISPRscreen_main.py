...
# name configure
if 'name_configure' not in config or config['name_configure']=='':
    print('please give the experiment name configure file!')
    sys.exit()
else:
    name_configure=config['name_configure']
...
os.system('python CRISPRscreen_sgRNA.py %s %s %s %s %s %s 2>>error.log'%(experiment_configure,name_configure,normalizedData,control_setting,prefix,ReadsThreshold))
...
os.system('python CRISPRscreen_gene.py %s %s %s %s %s %s %s %s %s %s %s %s 2>>error.log'%(geneDic,geneZDic,geneLst,sgRNALst,stressed_conditionLst,gene_sgRNA_position,control_setting,hit_gene_calling,FDR_threshold,prefix,sgRNAperQuasiGene,min_sgRNAnumber))
...
if len(Operon_gene_List)!=0:
    ...
    print ('operon statistics calculation finalized')
