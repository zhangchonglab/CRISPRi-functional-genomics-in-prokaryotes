    ...
    for condition in Result_geneDic:
        ...
        for gene in Result_geneDic[condition]:
            if gene in OperonDic[operon]:
                ...
                Result_OperonDic[condition][operon][gene].append(FPRvalue)
...
# ////////////////////////////////
os.system('mkdir -p %s_results/%s_operon_statistics/'%(prefix,prefix))
...
for condition in Result_OperonDic:
    ...
    with open('%s_results/%s_operon_statistics/%s_%s_operon_statistics.txt'%(prefix,prefix,prefix,condition),'r+') as f: 
        ...
        for operon in Result_OperonDic[condition]:
            ...
            FDRvalue=''
            FPRvalue=''
            for gene in OperonDic[operon]:
                geneName+=gene+','
                if gene not in Result_OperonDic[condition][operon]:
                    medianRAC+='ND,'; medianZ+='ND,'; negativeLog10P+='ND,'; sgRNAnumber+='ND,'; FDRvalue+='ND,'; FPRvalue='ND,'
                else:
                    ...
                    FPRvalue+=str(Result_OperonDic[condition][operon][gene][5])+','
            f.write('%s\t%s\t%s\t%s\t%s\t%s\t%s\n'%(geneName.strip(','), sgRNAnumber.strip(','), medianRAC.strip(','), medianZ.strip(','), negativeLog10P.strip(','), FDRvalue.strip(','), FPRvalue.strip(',')))
