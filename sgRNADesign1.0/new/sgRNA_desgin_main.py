...
def metricJudge(varlist):
    ...
    gccontentmax =True if float(varlist['gccontentmax'])<100 and float(varlist['gccontentmax'])>0 and float(varlist['gccontentmax'])>float(varlist['gccontentmin']) else False
...
def blastfileprocess(blastfile):
    ...
    with open(blastfile)as f:
        for line in f.readlines():
            ID1=line.split('\t')[0].split(':')[1].split('|')[0]
            ID2=line.split('\t')[1].split(':')[1].split('|')[0]
...
def construct_two_layer(targetDic,clusterLst_trans,clusterGroupLst_trans):
    ...
    if len(clusterGroupLst_trans)==0:
        geneID_clusterLst=geneID_clusterLst
    else:
        geneID_clusterLst=clusterGroupLst_trans+geneID_clusterLst
    return geneID_clusterLst
...
def singlecopy(varlist):
    if ".rnt" in varlist['indexfile']:
        genePositionLst,genePositionDic=rntprocess(varlist['indexfile'])
    else:
