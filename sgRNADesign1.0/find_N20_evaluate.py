...
gene_seqLst=[item.upper() for item in sys.argv[2].split('-')]
genome=sys.argv[3]
...
spacer_length=20
PAM='NGG'
copy_number=len(gene_seqLst)
multiple=True if sys.argv[10]=='True' else False
...
def sgRNA_judge_muti(offtarget_penalty,copy_number,off_threshold):
    offtarget_penalty.sort()
    flag=True
    if len(offtarget_penalty)<2*copy_number:
        return False
    elif len(offtarget_penalty)==2*copy_number:
        for penalty in offtarget_penalty:
            if penalty!=0:
                flag=False
        return flag
    elif offtarget_penalty[2*copy_number]>off_threshold: # the best offtarget has big penalty
        for penalty in range(2*copy_number):
            if penalty!=0:
                flag=False
        return flag # this sgRNA is ok
    else:
        return False
...
if sgRNA_existence_flag:
    ...
    for ID in sgRNA_ID_collection:
        ...
        if multiple:
            judgement=sgRNA_judge_muti(Candidate_N20mismatchDic[sgRNA_ID],copy_number,off_threshold)
        else:
            judgement=sgRNA_judge(Candidate_N20mismatchDic[sgRNA_ID],off_threshold)
        if judgement :  # pass the first threshold
